...
print(f'loading sdxl refiner......')
model_path = r'/mnt/nfs/file_server/public/mingjiahui/models/stabilityai--stable-diffusion-xl-refiner-1.0'
# from diffusers import StableDiffusionPipeline
# refiner = StableDiffusionPipeline.from_single_file(
#     f'{model_path}/sd_xl_refiner_1.0.safetensors',
#     config_file=f'{model_path}/model_index.json',
#     torch_dtype=torch.float16,
#     variant="fp16",
# )
# refiner.to("cuda")
from tool.model_util import from_single_file
refiner = from_single_file(
    f'{model_path}/sd_xl_refiner_1.0.safetensors',
    text_encoder_2=base.text_encoder_2,
    vae=base.vae,
    config_file=f'{model_path}/sd_xl_refiner.yaml',
    torch_dtype=torch.float16,
    variant="fp16",
)
refiner.to("cuda")
...
img_path = r'/mnt/nfs/file_server/public/mingjiahui/data/inference_test/000000000776.jpg'       # 632
...
def sample(i):
    ...
    # w refiner
    image.save(f'/home/mingjiahui/data/2_{i}_res_refiner.jpg')
    ...
    #     revision=None,
    # )
    res = base.vae.decode(base_output/ base.vae.config.scaling_factor, return_dict=False)[0]
    res = (res / 2 + 0.5).clamp(0, 1)
    res = tensor2img(res)

    print(type(res))
    print(res.shape)
    cv2.imwrite(f'/home/mingjiahui/data/2_{i}_res_base.jpg', res)
